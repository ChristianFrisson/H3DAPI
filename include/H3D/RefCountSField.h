...
namespace H3D {
  ...
  protected:
    ...
    virtual void onAdd( RefClass *n ) {
      ...
      Console(LogLevel::Debug) << "RefCountSField: Add node" << endl;
#endif
    }
...
#ifdef REF_COUNT_DEBUG
      Console(LogLevel::Debug) << "RefCountSField: Remove node" << endl;
#endif
    }
...
#ifdef REF_COUNT_DEBUG
    Console(LogLevel::Debug) << "RefCountSField(" << name << ")::update()   event_ptr = " 
               << event.ptr << endl;
#endif
...
#ifdef REF_COUNT_DEBUG
    Console(LogLevel::Debug) << "RefCountSField(" << name << ")::setValue()" << endl;
#endif
    value = n;
    // generate an event.
    this->startEvent();
...
#ifdef REF_COUNT_DEBUG
    Console(LogLevel::Debug) << "RefCountSField(" <<  name << ")::getValue()" << endl;
#endif
