  ...
  for( unsigned int i = 0; i < texture->size(); i++ ) {
    ...
    if( arg2 == "DIFFUSE" ) {
      ...
      if( arg2 == "" ) {
        if( previous_func == "COMPLEMENT" ) {
          ...
          if( previous_func != "" ) {
            cerr << "Warning: Invalid function \"" << previous_func 
                 << "\" in MultiTexture "
                 << " node (" << getName() << "). " << endl; 
          }
        }
        ...
        if( arg2 == "SPECULAR" ) {
          cerr << "Warning: Unsupported source \"" << arg2 
               << "\" in MultiTexture "
               << " node (" << getName() << ")." << endl; 
        } else {
    ...
    if( rgb_blend_mode == "MODULATE" ) {
      ...
      if( rgb_blend_mode == "ADDSMOOTH" ||
        ...
        cerr << "Warning: Unsupported mode \"" << rgb_blend_mode 
             << "\" in MultiTexture "
             << " node (" << getName() << "). Using MODULATE instead. " << endl; 
      } else {
        cerr << "Warning: Invalid mode \"" << rgb_blend_mode 
             << "\" in MultiTexture "
             << " node (" << getName() << "). Using MODULATE instead. " << endl; 
      }
    }
    ...
    if( alpha_blend_mode == "MODULATE" ) {
      ...
      if( alpha_blend_mode == "ADDSMOOTH" ||
        ...
        cerr << "Warning: Unsupported mode \"" << alpha_blend_mode
             << "\" in MultiTexture "
             << " node (" << getName() << "). Using MODULATE instead. " << endl; 
      } else {
        cerr << "Warning: Invalid mode \"" << alpha_blend_mode
             << "\" in MultiTexture "
             << " node (" << getName() << "). Using MODULATE instead. " << endl; 
      }
    }
