...
void ShaderProgram::initCGShaderProgram() {
  ...
  if( cgGLIsProfileSupported( cg_profile ) ) {
    for( MFString::const_iterator i = url->begin(); 
      ...
      if( resolved_url != "" || url_contents != "" ) {
        ...
        if( err == CG_NO_ERROR ) {
          ...
          Console(LogLevel::Warning) << "Warning: Shader program error when compiling source (" 
                     << *i << ") of \"" 
    ...
    if( getURLUsed() == "" ) {
      Console(LogLevel::Error) << "Warning: None of the urls in ShaderProgram node with url [";
      for( MFString::const_iterator i = url->begin(); 
           i != url->end(); ++i ) {  
