  ...
  if( proto_declaration.get() ) {
    ...
    for( XMLSize_t i = 0; i < nr_attrs; ++i ) {
      ...
      if( !field_name && name == "name" ) {
        ...
        Console(LogLevel::Warning) << "Warning: Unknown attribute \"" << name  
    ...
    if( x3d_type == X3DTypes::UNKNOWN_X3D_TYPE ) {
      Console(LogLevel::Warning) << "Warning: Invalid value for \"type\" attribute of \"field\" element "
      ...
      string error_message = proto_declaration->addFieldDeclaration(
                              ...
                              v );
                        if( error_message != "" ) {
                                Console(LogLevel::Warning) << error_message << " " << getLocationString() << endl;
                        }
    }
...
void X3DSAX2Handlers::handleConnectElement( const Attributes &attrs, 
  ...
  if( !defining_proto_connections ) {
    Console(LogLevel::Warning) << "Warning: \"connect\" element can only be defined in a IS element "
         << getLocationString() << endl;
    ...
    if( !node_field_name ) {
      ...
      if( !node_field ) {
        Console(LogLevel::Warning) << "Warning: Could not find field \"" << node_field_name  
             << "\" in parent node" << getLocationString() << endl;
        return;
      }
      
      if( !proto_field ) {
        Console(LogLevel::Warning) << "Warning: Could not find field \"" << proto_field_name  
      ...
      if( node_field->getAccessType() != Field::INPUT_OUTPUT &&
        ...
        Console(LogLevel::Warning) << "Warning: accessType of \"nodeField\"("
      ...
      if( node_field->getX3DType() != proto_field->getX3DType() ) {
        Console(LogLevel::Warning) << "Warning: type of \"nodeField\" and \"protoField\" does not match"
             << getLocationString() << endl;
        return;
      }
                        ...
                        for( Field::FieldSet::const_iterator i = routes_out.begin();
                                ...
                                if( (*i)->getOwner() == proto_instance ) {
                                        Console(LogLevel::Warning) << "Warning: \"nodeField\"("
                                                                                 << node_field->getName()
...
void X3DSAX2Handlers::protoStartElement( const XMLCh* const uri,
  ...
  if( defining_proto_body > 0 ) {
    if( defining_proto_connections ) {
      Console(LogLevel::Warning) << "Warning: Only \"connect\" elements allowed in IS element "
                 << getLocationString() << endl;
      node_stack.push( NodeElement( NULL ) );
    } else {
    ...
    if( proto_declaration.get() ) {
      Console(LogLevel::Warning) << "Warning: ProtoDeclare element not allowed inside other ProtoDeclare element"
           << getLocationString() << endl;
      node_stack.push( NodeElement( NULL ) );
    } else {
      const XMLCh *name = attrs.getValue( gname );
      if( !name ) {
        Console(LogLevel::Warning) << "Warning: ProtoDeclare element is missing required \"name\" attibute "
             << getLocationString() << endl;
        node_stack.push( NodeElement( NULL ) );
      } else {
    ...
    if( !proto_declaration.get() || defining_proto_interface || 
      ...
      Console(LogLevel::Warning) << "Warning: ProtoInterface element can only be a child element of ProtoDeclare element "
           << getLocationString() << endl;
      node_stack.push( NodeElement( NULL ) );
    } else {
    ...
    if( !proto_declaration.get() || defining_proto_interface || 
      ...
      Console(LogLevel::Warning) << "Warning: ProtoBody element can only be a child element of ProtoDeclare element "
           << getLocationString() << endl;
      node_stack.push( NodeElement( NULL ) );
    } else {
    ...
    if( defining_proto_interface || defining_extern_proto ) { 
      if( localname_string == "field" ) {
        ...
        Console(LogLevel::Warning) << "Warning: Only \"field\" elements allowed in ProtoInstance element "
             << getLocationString() << endl;
        node_stack.push( NodeElement( NULL ) );
      }
    }
...
X3DPrototypeInstance* X3DSAX2Handlers::handleProtoInstanceElement( const Attributes &attrs ) {
  ...
  if( name ) {
    ...
    if( proto ) return proto->newProtoInstance();
      ...
      Console(LogLevel::Warning) << "Warning: Could not find PROTO declaration for \"" << s_name  
           << "\" instance (" << toString( locator->getSystemId() )
           << ", line " << locator->getLineNumber() << ")" << endl;
    }
  } else {
    Console(LogLevel::Warning) << "Warning: ProtoInstance element is missing required \"name\" attibute ("
         << toString( locator->getSystemId() )
...
void X3DSAX2Handlers::handleExternProtoDeclareElement( const Attributes &attrs ) {
  if( proto_declaration.get() ) {
    Console(LogLevel::Warning) << "Warning: ExternProtoDeclare element not allowed inside other ProtoDeclare element"
         << getLocationString() << endl;
    node_stack.push( NodeElement( NULL ) );
  } else {
    const XMLCh *name = attrs.getValue( gname );
    ...
    if( !name ) {
      Console(LogLevel::Warning) << "Warning: ExternProtoDeclare element is missing required \"name\" attibute "
           << getLocationString() << endl;
      node_stack.push( NodeElement( NULL ) );
    } else if( !url ) {
      Console(LogLevel::Warning) << "Warning: ExternProtoDeclare element is missing required \"url\" attibute "
           << getLocationString() << endl;
      node_stack.push( NodeElement( NULL ) );
    } else {
      vector< string > urls;
      ...
      if( url_used == "" ) {
        Console(LogLevel::Warning) << "Warning: None of the urls in ExternProtoDeclare element could "
      ...
      if( proto_name == "" ) {
        ...
        if( !pd ) {
          Console(LogLevel::Warning) << "Warning: Could not find any PROTO declaration "
               << getLocationString()<< endl;
          node_stack.push( NodeElement( NULL ) );
        }
      } else {
        ...
        if( !pd ) {
          Console(LogLevel::Warning) << "Warning: Could not find PROTO declaration \"" 
               << proto_name
               << "\" " << getLocationString()<< endl;
          node_stack.push( NodeElement( NULL ) );
        }
...
void X3DSAX2Handlers::handleFieldValueElement( const Attributes &attrs, 
  ...
  if( !pi ) {
    Console(LogLevel::Warning) << "Warning: fieldValue element can only appear inside ProtoInstance "
  ...
  if( name && value ) {
    ...
    if( !field ) {
      Console(LogLevel::Warning) << "Warning: Field named \"" << name 
    ...
    if( !pfield ) {
      Console(LogLevel::Warning) << "Warning: Cannot parse field \"" << name 
           << "\". Field type must be a subclass of ParsableField "
  ...
  if( name_value_missing ) {
    Console(LogLevel::Warning) << "Warning: Both \"name\" and \"value\" attributes required "
...
void X3DSAX2Handlers::startCDATA() {
  ...
  cdata = "";
        if( proto_declaration.get() && !defining_proto_body ) {
                Console(LogLevel::Warning) << "Warning: CDATA elements not allowed inside ProtoInterface."
                                                         << getLocationString() << endl;
        }
}
...
//                             const XMLSize_t length ) {
   ...
   ///  if( length > 0 )
  //Console(LogLevel::Error) << "Comment: " << toString( chars ) << endl;
//}
  ...
  if( !parent && node_stack.size() > 0 ) {
    ...
    if( pi ) {
      ...
      if( localname_string == "fieldValue" ) {
                                ...
                                if( !proto_instance ) {
          Console(LogLevel::Warning) << "Warning: IS elements only allowed inside ProtoBody elements"
                                                                                 << getLocationString() << endl;
          node_stack.push( NodeElement( NULL ) );
        } else {
        ...
        Console(LogLevel::Warning) << "Warning: Only fieldValue ( and in some cases IS ) elements "
      ...
      if( localname_string == "fieldValue" ) {
        ...
        Console(LogLevel::Warning) << "Warning: ProtoInterface elements only allowed inside ProtoDeclare elements"
             << getLocationString() << endl;
        node_stack.push( NodeElement( NULL ) );
      } else if( localname_string == "ProtoBody" ) {
        Console(LogLevel::Warning) << "Warning: ProtoBody elements only allowed inside ProtoDeclare elements"
             << getLocationString() << endl;
        node_stack.push( NodeElement( NULL ) );
      } else if( localname_string == "IS" )  {
        if( !proto_instance ) {
        ...
        if( use_name ) {
          ...
          if( localname_string == "ProtoInstance" ) {
            ...
            try {
              ...
              if( !new_node ) {
                Console(LogLevel::Warning) << "Warning: Could not create \"" << localname 
          ...
          for( XMLSize_t i = 0; i < nr_attrs; ++i ) {
            ...
            if( name == "use" ) {
              ...
              if( dynamic_cast< PythonScript * >(parent) &&
                ...
                Console(LogLevel::Warning) << "Warning: When using a PythonScript in a Protobody "
              ...
              if( !field ) {
                Console(LogLevel::Warning) << "Warning: Couldn't find field named \"" << name 
                           << "\" in " << qname << " node " 
                           << getLocationString() << endl;
              } else {
                ParsableField *pfield = 
...
void X3DSAX2Handlers::warning(const SAXParseException& e) {
  Console(LogLevel::Warning) << "Warning: " << e.getMessage() << " (" << e.getSystemId()
             << ", line " << e.getLineNumber()
