...
void ShaderImage3D::render ( ){
  ...
  // image_unit_id will be the reference that shader used to access this image
  glBindImageTextureEXT ( image_unit, texture_unit, 0, true, 0, GL_READ_WRITE, stringImageFormat_map[imageFormat->getValue ( )] );
}

void ShaderImage3D::prepareShaderImage ( unsigned int texture_unit_id ){
  if( !texture_id ) {
