...
uniform vec3 unf34;
const float cons38 = float(29.999983);
const float cons48 = float(0.800000);
uniform vec4 unf51;
const float cons60 = float(50.000000);
const float cons62 = float(1.000000);
const float cons63 = float(0.500000);
uniform vec3 unf68;
const vec3 cons69 = vec3(1.000000, 1.000000, 1.000000);
const float cons78 = float(1.000000);


// h3d custom vars begin
uniform mat4 view_matrix;
uniform mat4 unf34_transform;
// h3d custom vars end
...
void main(void)
{
	...
	shade_view(oPosition, tmp2);
	texco_uv(oTexCoord, tmp4);
	mtex_normal(tmp4, samp0, tmp7);
	vec_math_negate(oNormal, tmp9);
	mtex_nspace_tangent(oTangent, tmp9, tmp7, tmp13);
	texco_uv(oTexCoord, tmp15);
	mtex_image(tmp15, samp1, tmp18, tmp19);
	set_rgba(tmp19, tmp21);
	mtex_alpha_from_col(tmp19, tmp23);
	srgb_to_linearrgb(tmp21, tmp25);
	mtex_rgb_blend(cons26, tmp25.rgb, tmp23, cons29, tmp30);
	set_rgb_zero(tmp31);
	set_rgb_zero(tmp32);
	lamp_visibility_other(oPosition, (view_matrix * unf34_transform * vec4(unf34.x, unf34.y, unf34.z, 1.0)).xyz, tmp35, tmp36, tmp37);
	lamp_falloff_invsquare(cons38, tmp36, tmp40);
	lamp_visibility_clamp(tmp40, tmp42);
	shade_inp(tmp13, tmp35, tmp45);
	shade_visifac(tmp45, tmp42, cons48, tmp49);
	shade_mul_value(tmp49, unf51, tmp52);
	shade_madd_clamped(vec4(tmp31, 1.0), tmp52, vec4(tmp30, 1.0), tmp56);
	shade_cooktorr_spec(tmp13, tmp35, tmp2, cons60, tmp61);
	shade_spec_t(cons62, cons63, tmp42, tmp61, tmp66);
	shade_add_spec(tmp66, unf68, cons69, tmp70);
	shade_add_clamped(vec4(tmp32, 1.0), vec4(tmp70, 1.0), tmp73);
	shade_add(tmp56, tmp73, tmp76);
	mtex_alpha_to_col(tmp76, cons78, tmp79);
	shade_alpha_opaque(tmp79, tmp81);
	linearrgb_to_srgb(tmp81, tmp83);
